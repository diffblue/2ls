...
protected:
  ...
  std::list<exprt> transform_pointed_params_in(
    const std::list<exprt> &params_in);
  std::list<exprt> transform_pointed_args_in(
    const std::list<exprt> &args_in,
    const local_SSAt &SSA,
    local_SSAt::locationt loc);

  std::list<exprt> transform_pointed_member_params_in(
    const std::list<exprt> &params_in,
    const struct_union_typet::componentt &component);

  std::list<exprt> transform_pointed_member_args_in(
    const std::list<exprt> &args_in,
    const struct_union_typet::componentt &component,
    const local_SSAt &SSA,
    local_SSAt::locationt loc);
  std::list<exprt> transform_pointed_params_out(
    const std::list<exprt> &params_out,
    const local_SSAt::var_sett &globals_out,
    const typet &param_type);
  std::list<exprt> transform_pointed_args_out(
    const std::list<exprt> &args_out,
    const typet &arg_symbol_type,
    const typet &param_type,
    const local_SSAt &SSA,
    local_SSAt::locationt loc);
  std::list<exprt> transform_pointed_member_params_out(
    const std::list<exprt> &params_out,
    const struct_union_typet::componentt &component,
    const local_SSAt::var_sett &globals_out);
  std::list<exprt> transform_pointed_member_args_out(
    const std::list<exprt> &args_out,
    const struct_union_typet::componentt &component,
    const local_SSAt &SSA,
    local_SSAt::locationt loc);
