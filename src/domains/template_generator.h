...
#ifndef DELTACHECK_TEMPLATE_GENERATOR_H
#define DELTACHECK_TEMPLATE_GENERATOR_H
...
class template_generatort : public messaget
{
public:
  typedef strategy_solver_baset::var_listt var_listt;

  explicit template_generatort(
    optionst &_options, 
		ssa_dbt &_ssa_db,
...
protected:
  ...
  void collect_variables_loop(
    domaint::var_specst &var_specs,
    const local_SSAt &SSA,
    bool forward);
  void collect_input_variables_callingcontext(
    domaint::var_specst &var_specs,
    const local_SSAt &SSA,
    local_SSAt::nodest::const_iterator n_it,
    local_SSAt::nodet::function_callst::const_iterator f_it);
  void collect_output_variables_callingcontext(
    domaint::var_specst &var_specs,
    const local_SSAt &SSA,
    local_SSAt::nodest::const_iterator n_it,
    local_SSAt::nodet::function_callst::const_iterator f_it);
  void collect_variables_in(
    domaint::var_specst &var_specs,
    const local_SSAt &SSA,
    bool forward);
  void collect_variables_out(
    domaint::var_specst &var_specs,
    const local_SSAt &SSA,
    bool forward);

  void filter_template_domain(domaint::var_specst &var_specs);
  void filter_equality_domain(domaint::var_specst &var_specs);
  ...
  void instantiate_standard_domains(
    domaint::var_specst &var_specs,
    const local_SSAt &SSA);
